...
You can find the source code at https://github.com/xpipe-io/xpipe-python-api and the code of the samples used on this page at https://github.com/xpipe-io/xpipe-python-api/tree/master/samples.
...
In the context of the API, XPipe refers to any kind of connection, identity, script, and others as a `store`. This just refers to the fact that those entries store json configuration data and some metadata.

In general, the process of adding, editing, and removing stores is the same, regardless of the type of store. Only the json data and the capabilities of the store will differ.
...
schema = client.store_info([store])[0]["config"]
```
...
```python
encrypted_secret = client.secret_encrypt_default("s3cret")
...
When specifying information about any kind of password authentication, you will have to encrypt more than just a raw password. As XPipe supports many different ways of retrieving secrets for password authentication, you will have to encrypt this information as well.
...
For SSH, anything like key references also need to be encrypted. With that, you can now create store schemas and add them like this:
...
To store an SSH key in-place, you can do this:
...
Of course, there are also more variants for SSH keys. To inspect the key information of an existing store, you can use the `secret_decrypt` function on the identity data of a connection.
...
The previous example created connections with an inline identity, i.e. one that was not created as a separate entry. For identities that will only be assigned to one connection, this is usually good enough. If you intend to assign the same identity to multiple systems, using reusable identities is the better choice.

### Using reusable identities

Once you have created an identity in XPipe, you can then just use the identity UUID as a reference:
...
You can also create identities through the same API and with the same encrypted values for the password and SSH key:
...
### Creating synced identities

You can do the same with synced identities. The difference here is that you can control who has access in the case of team vaults.
...
### Creating password manager identities

And finally, here is how to create password manager identities:
