...
You can create a new service manually at `New` -> `Service ...`. You can configure the necessary service information plus a few additional properties. The exact configuration options depend on how you intend to use the service: Tunnel a remote port on an existing host to your local machine or just open a remote service without tunneling involved. Whether you have control over the tunneling settings depends on the nature of the service and whether there is a gateway involved or not.
