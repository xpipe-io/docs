...
XPipes comes with integrations for almost every terminal tool out there, so chances are high that you can keep using your favourite terminal setup in combination with XPipe.
...
On Windows, you can dock terminals to the XPipe application window. When enabled, XPipe will then manage and resize the terminal window to fit into the connection hub:

![Dock](/images/dock.png)

A docked terminal is fixed to the XPipe window but can also be detached from it. If you move the terminal window away manually, e.g. by dragging it or using keyboard shortcuts, it enters a detached state where XPipe no longer manages the terminal.

You can also minimize the terminal normally, plus also toggle the terminal with the terminal dock button that will show up at the bottom right when a terminal is active:

![Dock toggle](/images/dock-toggle.png)

The dock button will also change the icon depending on the terminal state, e.g. when it is detached or minimized.

This button also has the shortcut of CTRL+T assigned to it, meaning that you can show the terminal again with CTRL+T while XPipe has focus. Note that you can't use the same shortcut to hide the terminal again unless the XPipe window has focus as usually the terminal becomes focused once it gets shown.

If you want to disable the terminal docking completely, you can do so in the settings menu:

![Dock setting](/images/dock-setting.png)

### Window focus

The terminal dock might feel a little weird at first due to how it works with controlling the terminal window. The main thing to keep in mind is that XPipe manages the terminal window, not the other way around. This means that if you want to move the windows to another position, you will have to drag the XPipe window, not the terminal window. The XPipe window then automatically moves the terminal for you if it is attached.

To control both XPipe and terminal with other operations like keyboard shortcuts, you will first have to make sure that the XPipe window has focus. If the terminal window has focus, the XPipe window will not receive those shortcuts and the terminal becomes detached.

In this example, you can see that the left XPipe window has focus, so it will receive any input. The right XPipe window does not have focus as the window frame is greyed out, meaning that the terminal will receive input:

![](/images/dock-focus.png)
