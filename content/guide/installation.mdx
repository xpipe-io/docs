...
<Callout type="info">If you are looking to use a package manager or want to deploy XPipe in your organization and want to automate the installation and upgrade process across multiple systems, check out the [managed installation docs](/guide/managed-installation) instead.</Callout>

<Callout type="info">Note that this is a desktop application that should be run on your local desktop workstation, not on any server or containers. You don't need to set up anything on any servers with XPipe. All commands you see here should be run on your local desktop system.</Callout>
...
- [scoop](https://scoop.sh) to install it with `scoop install extras/xpipe`.

See the [managed installation docs](/guide/managed-installation) for more advanced installation methods.
...
XPipe can also be run in the Windows Subsystem for Linux with graphics support. If you follow the instructions for [WSL2g GUIs](https://learn.microsoft.com/en-us/windows/wsl/tutorials/gui-apps), installing XPipe on the WSL system via one of the available Linux installers will make XPipe run in your WSL environment.
...
If you are running an up-to-date Android system with support for AVF, you can take a look at the new [Android Linux Terminal](https://www.zdnet.com/article/how-to-use-the-new-linux-terminal-on-android/) app. This official app essentially runs a full debian VM on your phone and does not require any sideloading or jailbreaks. If you are running Android 16+ and your phone supports AVF (not all do, even newer ones), you can use it.

XPipe can be run on this debian system by just installing it as a normal application as you would do on a regular Linux system. See the [debian install instruction](#debian-based-distros) for details.
...
Alternatively, you can also use the official [homebrew tap](https://github.com/xpipe-io/homebrew-tap) to install XPipe with `brew install --cask xpipe-io/tap/xpipe`.
