...
This is suitable if you are using a niche agent that is not supported out-of-the-box by XPipe.
...
#### Other external auth [!toc]

This option will permit any external running identity provider to supply its keys to the SSH client. You should use this option if you are using any other type of password manager agent or other type of auth configured in your local SSH config file.

XPipe will not interact with any external agent in this case, it will just allow any configured keys and external program to supply keys to the SSH client. You will have to make sure that the agent you want to use is running beforehand and configured correctly.

#### Other auth types

In case your authentication method is not covered at all by the available predefined methods, you can still choose to use the [additional SSH options](#adding-ssh-options) field to supply any arbitrary auth config data to SSH.

For example, if you are using something like [OpenBao](https://openbao.org/docs/secrets/ssh/signed-ssh-certificates/#client-ssh-authentication) to authenticate via SSH, you will have to use the additional options field to pass two key files:

![](/images/ssh-openbao.png)
...
SSH connections can be augmented with SSH config options if needed. This bridges the gap between basic SSH connections and fully custom SSH connections, which might be overkill for some cases. If you are just looking to add a simple option for a connection, like accepting a deprecated host key algorithm, you can add this in the advanced configuration:
