...
Whether or not the integration requires the root user to interact with the respective daemon depends on your individual setup, in most cases, whether your user is added to the authorized LXD/incus groups or not. If your user is not a member of that group, root is required. Both rootless and root variants are supported, and XPipe will prompt you for your sudo password if needed. For more information, see the incus documentation on [access to the daemon](https://linuxcontainers.org/incus/docs/main/explanation/security/#local-access-to-the-incus-daemon).

## Daemon

The LXC/incus daemon must be running for the integration to work. If it is not running, you have to start the daemon service first.
