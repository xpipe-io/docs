...
For Windows, the mstsc integration comes with its own tabbed docking window as mstsc itself is missing this functionality:
...
The basic form of RDP connections are direct RDP connections to a system without an SSH tunnel to the target system itself:
...
You can also create and use remote desktop gateway servers for RDP:

![](/images/rdg.png)

You can control how the authentication for the gateway should be handled, depending on your individual configuration.

After creating an RD gateway entry, you can set it as a gateway for your RDP connection. SSH gateways via tunneling are also supported for RDP, you can choose between both of these gateway types in the gateway field:

![](/images/rdg-config.png)

### SSH tunnel gateways

As an alternative to remote desktop gateways, you can also choose to tunnel the traffic over an intermediate SSH tunnel:

![](/images/rdg-tunnel.png)

This allows you to connect to RDP servers behind some form of bastion server, for example, without requiring a proper remote desktop gateways server.

## Tunneled RDP connections

If your target RDP server is also running an SSH server, you can connect via this SSH server connection instead. The remote RDP port is then tunneled over SSH to your local machine. The advantage of this is that the RDP port doesn't have to be publicly exposed, and you can reuse authentication from an SSH connection. Furthermore, you will gain access to the more advanced SSH authentication options to connect to your RDP server compared to the often times basic password authentication for RDP.
