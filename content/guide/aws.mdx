...
If you want to [sync](/guide/sync) your AWS connections across multiple devices or share them with other team members, you will have to differentiate between internally managed profiles and external CLI config profiles. An internally managed profile can be synced automatically. A file-based profile requires the config file to be synced.
