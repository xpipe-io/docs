...
If you want to [sync](/guide/synchronization) your AWS connections across multiple devices or share them with other team members, you will have to differentiate between internally managed profiles and external CLI config profiles. An internally managed profile can be synced automatically. A file-based profile requires the config file to be synced.

To also sync the config file and its profiles, you will have to edit the configuration of the top-level AWS connections entry and toggle the sync button:
