...
By default, a docker container does not have access to the network tunnel driver ([TUN](https://en.wikipedia.org/wiki/TUN/TAP)) or the host system. For most types of connections, this one is not needed, so it will work fine without it. However, tools like Netbird require the TUN to function within the container.
