...
The network stack on a guest system also takes a few seconds to initialize all the network interfaces and to obtain an IP. If the libvirt daemon is unable to tell the guest VM IP address, the guest agent is not running, the networking is not initialized yet, or you are running some kind of custom networking or firewall setup. See the [custom networking section](#custom-networking) for handling this configuration.
