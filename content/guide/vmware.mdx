...
<Callout type="info">Linked remote VMs like remote VMware ESXi or VMware vCenter Server instances are not supported. Since VMware is going down the drain, this support is also not planned to be added in the future.</Callout>
...
- The VM has to be able to advertise its IP via the VMware tools/open-vm-tools (or you need to provide a manual IP override)
