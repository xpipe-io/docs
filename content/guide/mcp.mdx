...
If you haven't heard about it before, the Model Context Protocol (MCP) is an open standard developed by Anthropic. The core idea is to give AI agents a consistent way to connect with tools, services, and data, no matter where they are or how they’re built. If you are using any AI-powered application like Cursor, Claude Desktop, or others, you can get started right away as they all support MCP.
...
Note that the streamable HTTP transport protocol is required. This is relatively new and might not be supported by all clients yet. If your MCP client only supports the SSE transport, then you won't be able to connect to the XPipe MCP server. If you can only enter the URL and do not have an option to choose between different protocols, it is likely that only SSE is supported. XPipe will show an error message if a client attempts to connect via SSE.
