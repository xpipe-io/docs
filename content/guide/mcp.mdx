...
If you haven't heard about it before, the Model Context Protocol (MCP) is an open standard developed by Anthropic. The core idea is to give AI agents a consistent way to connect with tools, services, and data, no matter where they are or how they’re built. If you are using any AI-powered application like Cursor, Claude, or others, you can get started right away as they all support MCP.

XPipe provides an MCP server on a local HTTP server that an MCP client of your choice can connect to. Once connected, the MCP client can connect to remote systems, interact with file systems, run commands, and more. How much the agent can do depends on the amount of privileges you give the MCP server in XPipe. You will have to explicitly enable every kind of actions that can potentially modify a system.
...
When first starting out with these tools, make sure that your MCP client is configured to confirm any external tool invocations:
...
The MCP server also offers the tool `call_api`, which allows the MCP client to perform an API call to the HTTP API. This essentially allows the MCP client to add, modify, or remove connection entries from the vault. See the [API docs](/api) for details.

An MCP client usually retrieves the OpenAPI document when it is instructed to use the XPipe API with a prompt like `Add SSH connection to 192.168.1.1 to XPipe via the HTTP API`:

![](/images/mcp-api.png)

For advanced usage, it might also make sense to just utilize the [Python API](/guide/python-api) with an AI agent of your choice to create reusable scripts, e.g. to migrate a lot of connection information to XPipe via the API.
