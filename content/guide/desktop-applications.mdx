...
For graphical systems, XPipe implements a concept called coherent desktops. Essentially, you can launch desktop applications on a remote system directly from XPipe, regardless of the underlying connection implementation. In combination with the tunneling and background shell session support, you can launch graphical remote applications with one click in the same unified way for:
...
Here is an example of a Windows 11 system displaying a remote window of XPipe via X11 forwarding and a remote explorer.exe of a Windows Server, both via the desktop application integration:
