...
In some cases, especially when dealing with a lot of connections in a category with the same gateway like a bastion server, you don't have to assign a gateway to each connection separately. Instead, you can configure a default gateway for the whole category, which will be then automatically assigned to all newly added connections in that category:

![](/images/category-gateway.png)
...
XPipe has three ordering modes for connections: **Manual**, **Last used**, **Alphabetical**. You can see the currently active ordering mode in the status bar:

![](/images/sort-status.png)

In the screenshot, you can see that the order is set to manual. You can toggle the ordering type and also switch the order between ascending and descending by clicking on them again.

In manual ordering mode, you have the ability to rearrange connections by drag and drop:

![](/images/sort-drag.png)

You can't do this in the other ordering modes as their order is non-customizable.

You can, however, also drag and drop connections to other categories, regardless of the ordering mode. The same goes for ordering individual categories next to each other:

![](/images/sort-categories.png)

If you select multiple connections via [batch mode](#batch-mode), you can drag and drop many connections at once:

![](/images/sort-batch.png)

## Connection notes

For each connection, you have the ability to add custom notes. This can be any kind of information that you want to keep track of for certain systems. To add notes, you can use this option in the context menu of a connection:

![](/images/notes-add.png)

A dialog will then open were you the current connection notes are previewed as markdown:

![](/images/notes-dialog.png)

The notes can be edited via your text editor by clicking on the edit button in the top right corner of the markdown content:

![](/images/notes-dialog-edit.png)

Existing notes can be shown later on by clicking on the notes button which is visible if a connection entry contains notes:

![](/images/notes-edit.png)

If you are rendering complex markdown syntax which is not considered the universal standard, some elements might not be rendered correctly. This can, for example, be a markdown-based diagram. In that case, it makes sense to open the markdown content straight in an external text editor that supports those advanced rendering features in a markdown preview. You can open the editor without showing the notes dialog within XPipe by shift-clicking the notes button instead.

If you're looking to securely store the notes content, you can take a look at the different [vault encryption](/guide/sync#vault-encryption) methods.
...
You can toggle the right sidebar with this button when compact mode is enabled:

![](/images/hub-tile-toggle.png)

## Icons

If you are looking to fully customize all your connections, assigning custom icons might be a good start. Any connection can be freely assigned any available icon:
...
The same can be done for categories as well, clicking on the icon will bring up the same dialog:
...
If you want to use other repositories or directories of icons, you can easily add them. The first time you open icon choice dialog, it will show a button to refresh the icon sources first. Whenever you modify the sources list, you will also have to refresh the icon storage again. XPipe does not automatically pull changes from icon repositories as this can be a time-consuming operation.
