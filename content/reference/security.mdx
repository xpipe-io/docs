...
The detailed encryption algorithm is an AES-128 GCM block-cipher with 12 IV bytes and 128 tag bits. The encryption keys are generated with a [Password-based key-derivation algorithm (PBKDF)](https://datatracker.ietf.org/doc/html/rfc8018) using SHA-256 HMAC. It is important that your supplied passphrase provides a high level of entropy in order to guarantee the best possible encryption.
...
In the XPipe Professional plan, you have the option to use a remote vault as a team vault where multiple team members can interact with the same connection data. Team vaults also allow each user to encrypt certain server identities with their own user passphrase, allowing only that user to decrypt and have access to certain login information. That way, a team of users can maintain their list of available servers while also maintaining control login information for each user separately.
