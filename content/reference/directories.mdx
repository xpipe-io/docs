...
## Installation data
* `%LOCALAPPDATA%\XPipe` or `%ProgramFiles%\XPipe` on Windows, depending on the type of installation
* `/opt/xpipe` on Linux
...
Any files they are being downloaded in the file browser are saved temporarily to the following directories
